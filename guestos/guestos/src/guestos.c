...
extern long long int num_instr_executed;
...
void set_defaults(void) {
	...
	system(command);
	memory_access_table = (int)malloc(heads*tracks*sectors*sizeof(int));
	num_virtual_blocks = heads*tracks*sectors;
	int i=0;
	for(;i<num_virtual_blocks;i++)
		memory_access_table[i] = -1;
}
