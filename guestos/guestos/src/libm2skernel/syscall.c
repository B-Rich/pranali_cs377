...
long long int IOCost(long long int blockNo, long long int curBlockNo) {
	long long int headNo = blockNo/(tracks*sectors);
	blockNo = blockNo%(sectors*tracks);
	long long int trackNo = blockNo/(sectors);
	blockNo %= sectors;
	curBlockNo = curBlockNo%(sectors*tracks);
	long long int curTrackNo = curBlockNo/(sectors);
	long long int curSectorNo = curBlockNo%(sectors);
	long long int cost = ((blockNo - curSectorNo) >= 0)? (blockNo - curSectorNo)*sectorCost: (sectors + blockNo - curSectorNo)*sectorCost;
...
void block_process(long long int diff) {
  ...
  my_insert(diff,isa_ctx, interrupt_list); // POINTER TO INTERRUPT_LIST MUST BE PASSED
  printf("Process with UID %d to be back at instruction %d\n", isa_ctx->uid, diff);
}
  ...
  case syscall_code_set_instruction_slice:
    {
      ...
      printf("executing syscall for changing instruction slice\n");
      fflush(stdout);		
      break;
    }
  case syscall_code_my_read_write:
    {
      printf("executing syscall for read/write to virtual disk\n");
      fflush(stdout);
      uint32_t pbuf;
      int count;
      int err,db_no,db_offset;
      FILE* fd;
      void *buf;

				
      /* Get parameters */
      int rw = (int)(isa_regs->ebx);
      count = (int)(isa_regs->ecx);
      pbuf = (uint32_t)(isa_regs->edx);
      db_no = (int)(isa_regs->esi);
      db_offset = (int)(isa_regs->edi);
      ...
      if(fd==NULL)
	{
      ...
      syscall_debug(" rw = %d, count = %d,pbuf=0x%x, db_no=%d, db_offset = %d\n",rw,count, pbuf, db_no,db_offset);
      buf = calloc(1, count);
      if (!buf)
	fatal("syscall read: cannot allocate buffer\n");      
      fseek ( fd , db_no*512+db_offset , SEEK_SET );
				
      if((db_no + (db_offset + count)/512) >= num_virtual_blocks)
	{
	  printf("The blocks to be accessed exceed disk capacity\n");
	  fflush(stdout);
	  exit( -1);
	}
      if(rw== 0) //'r'
	{
	  int i=db_no;
	  int cur_uid = isa_ctx->uid;
	  for(;i<=(db_no + (db_offset + count)/512);i++)
	    {
	      if(!(memory_access_table[i]==-1 || memory_access_table[i] == cur_uid))
		{
	  ...
	  long long int instr_diff = IOCost(last_visited_block, db_no);
...
void syscall_do() {
  ...
  if (syscode > 325) {
      ...
      case syscall_code_mmap:
	{
	  ...
	  /* Map */
	  retval = do_mmap(addr, len, prot, flags, guest_fd, offset);
	  break;
	}
