	...
	int size, void *buf, enum mem_access_enum access)
{
	...
	if (access == mem_access_read || access == mem_access_exec) {
		/*
		void retreive_page(mem, page, access) 
		function : returns page with data set properly
		-> ignore if valid_bit = 1
		-> else loads page into memory and sets it in data
		-> changes valid_bit to 1
		-> if init_flag = 0, return with data set NULL
		*/
		if (page->data)
	...
	if (access == mem_access_write || access == mem_access_init) {
		...
		-> else loads page into memory and sets it in data
		-> changes valid_bit to 1
		-> if init_flag = 0, callocs a new page and sets it in data 
		*/
		if (!page->data)
			page->data = calloc(1, MEM_PAGESIZE);
...
unsigned long count_page_in_memory = 0;
unsigned long global_block_offset; 

void retrieve_page(struct mem_t* mem, struct mem_page_t* page, enum mem_access_enum access) {
	...
	if (access == mem_access_read || access == mem_access_exec) {
		if(!page->init_flag) {
			...
			if(page->valid_bit) {
				...
				/*
				void page_fault_handler(mem, page)
				function : loads page from disk to main memory
				*/
				page_fault_handler(mem, page);
				return;
			}
		}
	}
	...
	if (access == mem_access_write || access == mem_access_init) {
		if(!page->init_flag) {
			/*
			void page_create_handler(mem, page)
			function : creates a new calloc or replaces another calloc with current page
			*/
			page_create_handler(mem, page);
			return;
		} else {
			if(page->valid_bit) {
				return;
...
void page_create_handler(struct mem_t* mem, struct mem_page_t* page) {
	if(count_page_in_memory < MAX_PAGE_IN_MEMORY) {
		count_page_in_memory++;
		page->data = calloc(1, MEM_PAGESIZE);
		page->init_flag = 1;
		page->valid_bit = 1;
		return;
		...
		/* void* swap_out_page()
		*/
		page->data = swap_out_page(); 
		memset(page->data, 0, MEM_PAGESIZE); 
		page->init_flag = 1;
		page->valid_bit = 1;
		return;
	}
...
void page_fault_handler(struct mem_t* mem, struct mem_page_t* page) {
	if(count_page_in_memory < MAX_PAGE_IN_MEMORY) {
		count_page_in_memory++;
		page->data = calloc(1, MEM_PAGESIZE);
		page->init_flag = 1;
...
void* swap_out_page() {
	mem_page_t* page; 
	//find the page
	//store disk location if new else write to existing
	return page->data;
}
